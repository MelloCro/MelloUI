...
-- Function to add mUI button to Game Menu
local gameMenuButtonAdded = false
local originalButtonPositions = {}

local function AddGameMenuButton()
    -- Create our button only once
    if not GameMenuButtonMUI then
        local menuButton = CreateFrame("Button", "GameMenuButtonMUI", GameMenuFrame, "GameMenuButtonTemplate")
        menuButton:SetText("mUI")
        menuButton:SetScript("OnClick", function()
            PlaySound(SOUNDKIT.IG_MAINMENU_OPTION)
            HideUIPanel(GameMenuFrame)
            if addon.ProfileInstaller then
                addon.ProfileInstaller:ShowInstaller()
            end
        end)
    ...
    local menuButton = GameMenuButtonMUI
    ...
    if not gameMenuButtonAdded then
        ...
        -- Position mUI button at the top (moved up by 100 pixels total)
...
-- Additional event frame for maintaining styles
