#!/usr/bin/env python3
"""
Build Media/QuestObjectiveData.lua for the Route module: where each quest's
...
Output (world coordinates as the client tables give them, rounded to yards):
  MelloUI_QuestObjectiveData = {
    [questID] = { { kind, "name", "alt", map, x1, y1, x2, y2, ... }, ... },
  }
  kind: 1 creature, 2 object, 3 item, 4 area; map: 0 Eastern Kingdoms, 1 Kalimdor
...
def main():
    ...
    ap.add_argument("--out", default=os.path.join(HERE, "..", "Media", "QuestObjectiveData.lua"))
    ...
    with open(os.path.abspath(args.out), "w", encoding="utf-8", newline="\n") as fh:
        ...
        fh.write("-- [questID] = { { kind, name, alt, map, x1, y1, x2, y2, ... }, ... }; kind 1 creature, 2 object,\n")
        fh.write("-- 3 item, 4 area; map 0 Eastern Kingdoms, 1 Kalimdor; world coordinates in yards.\n\n")
        fh.write("MelloUI_QuestObjectiveData = {\n")
        for qid in sorted(out):
            parts = []
            for kind, name, alt, pts in out[qid]:
                m = pts[0][0]
                coords = ",".join(f"{x},{y}" for pm, x, y in pts if pm == m)
                parts.append(f"{{{kind},{lua_str(name)},{lua_str(alt)},{m},{coords}}}")
            fh.write(f"\t[{qid}]={{{','.join(parts)}}},\n")
        fh.write("}\n")
