...
local STEPS = {
	...
	{ page = "UIModifications", point = "LeftEdgeCenter",
	  target = function(_, page) return page.headerSwitch end,
	  text = "Unlock the Windows: tick it and drag any window by its title, the minimap by its zone band, the tracker by its header, the damage meter and the chat anywhere. The mouse wheel scales a window while you drag. It works with the reskin off as well. Positions stay across reloads; Reset positions puts everything back." },
