...
local STEPS = {
	...
	{ page = "Home", point = "RightEdgeCenter",
	  ...
	  text = "A tile per module: its switch, whether it is on, and Open page. UI Modifications is the important one: the painted reskin of the whole interface and every feature that tunes it, all in one place." },
	...
	{ page = "UIModifications", point = "BottomEdgeCenter",
	  target = function(c) return c.window and c.window.dynamicButton or c.band end,
	  text = "Dynamic UI Modification, up in the title bar: the look of the reskin in one place. The borders and Kit Colours of every window, the backgrounds and backdrops, the parchment sheets and the minimap's shape, chosen on the interface itself with pictures." },
